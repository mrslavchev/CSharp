...
namespace SolarSystem
{
    class OrbitsCalculator : INotifyPropertyChanged
    {
        ...
        {
            /*
             * Initially it was decimal which is slow for calculations, plus for readibility its better to calculate the steps
             * insite the loop. Ints are faster than floating points, that's why it's faster to make int to 7200000 than the initial 
             * loop. Last thing, since Days / EarthRotationPeriod don't change during iterations, it's more clever to
            ...
            //}
            double step = 0;
            double rotationFratction = Days / EarthRotationPeriod;
            for (int i = 0; i <= 7200000; i++)
            {
                EarthRotationAngle = step * rotationFratction;
                step += 0.00005d;
            }
